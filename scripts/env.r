...
; TODO:
;
;	* Refresh file from url other than base. Need a manifest?
;

env: context [

	master: https://raw.githubusercontent.com/codebybrett/rebol2/master/scripts/

	base: either find [url!] to word! type? system/script/args [
		system/script/args
	][
		what-dir
	]
	...
	retrieve: funct [
		...
		if all [
			...
			make error! reform [{Could not retrieve} (mold pattern)]
		]
	...
	run: funct [
		...
		either script [
			...
			if any [
				...
				make error! reform [{Invalid script/text for} search-file]
			]
...
script-needs: funct [
	...
	if not parse needs [
		...
		make error! reform [{Could not parse script-needs near:} (copy/part pos 40)]
	]
]
