...
date: binding/custom/object [parser] [
	...
	as: make object! [
		...
		excel: funct [
			...
			string: rejoin [form date/year #"-" pad 2 date/month #"-" pad 2 date/day]
			if time: date/time [
				second: decimal: none
				parse/all form time/second [copy second to #"." copy decimal to end]
				if parse/all decimal [#"." some #"0"] [decimal: {}]
		...
		iso8601: funct [
			...
			string: rejoin [form date/year dsep pad 2 date/month dsep pad 2 date/day]
			if time: date/time [
				second: decimal: none
				parse/all form time/second [copy second to #"." copy decimal to end]
				if parse/all decimal [#"." some #"0"] [decimal: {}]
				append string rejoin [
					...
					either 0:00 = zone: date/zone [#"Z"] [
						rejoin [either positive? zone [#"+"] [#"-"] pad 2 zone/hour #":" pad 2 zone/minute]
					]
				]
			]
			string
	...
	from: make object! [
		...
		iso8601: funct [
			...
			if parse/all string [
				...
				(date: to date! rejoin [year {-} month {-} day])
				opt [pos:
					...
					(date/time: to time! rejoin [hour #":" minute #":" second #"." decimal])
					opt [
						[
							#"z" (zone: 0:00)
		...
		unspecified: funct [
			...
			if parse/all string parser/grammar/rule [
				parser/date
			]
		]
